...
pkmn_forms <- select(pkmn_forms, c(id, identifier))
...
pkmn_forms_filtered <- pkmn_forms_filtered %>%
  rename(pokemon_species_id = id, name = identifier)
...
```{r}
# Check for duplicate Name values in merged_moves
duplicate_names_in_merged_moves <- merged_moves %>%
  group_by(Name) %>%
  filter(n() > 1) %>%
  ungroup()

# Check for duplicate Name values in combined_pkmn
duplicate_names_in_combined_pkmn <- combined_pkmn %>%
  group_by(name) %>%
  filter(n() > 1) %>%
  ungroup()

# Print the duplicate Names
print(duplicate_names_in_merged_moves)
print(duplicate_names_in_combined_pkmn)
...
filtered_moves <- merged_moves %>%
  select(Name, pokemon_species_id, Move, Move_Usage, generation_id, type_id, power, pp, accuracy, priority, target_id, damage_class_id, effect_id, effect_chance, type1, type2, short_effect) %>%
...
```{r}
# Check to see if all Pokemon names were added correctly (ideally no names are excluded)
